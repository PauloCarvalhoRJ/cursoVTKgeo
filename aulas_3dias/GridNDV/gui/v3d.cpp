...
V3D::V3D()
{
    ...
    if( ! path_arq_dados.isEmpty() ){
        ...
        // Cria um array VTK para marcar as celulas como visiveis (>=1) ou (<= 0).
        vtkSmartPointer<vtkIntArray> visibilidade = vtkSmartPointer<vtkIntArray>::New();
        visibilidade->SetNumberOfComponents(1);
        visibilidade->SetName("Visibility");
        visibilidade->Allocate( numCelulas );
        ...
        for( int j = 0; j < nY; ++j){
            for( int i = 0; i < nX; ++i) {
                ...
                if( fnIsNDV( value ) )
                    visibilidade->InsertNextValue( 0 );
                else
                    visibilidade->InsertNextValue( 1 );
            }
        }
        ...
        // Thresholding para esconder as celulas que tem valor invalido
